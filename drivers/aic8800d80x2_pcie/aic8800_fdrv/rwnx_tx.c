                            ...
                            void *frame_start)
{
    ...
    if (rwnx_ipc_buf_a2e_init(rwnx_hw, &sw_txhdr->ipc_data, frame_start,
                              sw_txhdr->frame_len))// + 4))
        return -1;
	#endif

    /* Update DMA addresses and length in tx descriptor */
	desc->host.packet_len[0] = sw_txhdr->frame_len;// + 4;
    //desc->host.packet_addr[0] = sw_txhdr->ipc_data.dma_addr;
    desc->host.packet_addr = sw_txhdr->ipc_data.dma_addr;
...
void rwnx_tx_push(struct rwnx_hw *rwnx_hw, struct rwnx_txhdr *txhdr, int flags)
{
    ...
    //rwnx_data_dump("tx:", skb->data + sw_txhdr->headroom, skb->len - sw_txhdr->headroom);
	if (((sw_txhdr->desc.api.host.flags & TXU_CNTRL_MGMT) && ((*(skb->data+sw_txhdr->headroom)==0xd0) || (*(skb->data+sw_txhdr->headroom)==0x10) || (*(skb->data+sw_txhdr->headroom)==0xc0) || ((*(skb->data+sw_txhdr->headroom)==0x30)))) || \
		(sw_txhdr->desc.api.host.ethertype == 0x8e88) || (sw_txhdr->desc.api.host.ethertype == 0xb488)) {
		// 0xd0:Action, 0x10:AssocRsp, 0x8e88:EAPOL
		sw_txhdr->need_cfm = 1;
										  ...
										  struct rwnx_sw_txhdr *sw_txhdr)
{
	...
	sw_txhdr->desc.api.host.packet_addr[amsdu->nb] = dma_addr;
 ...
 *
 * returns true if buffer has been added as A-MDSU subframe, false otherwise
 *
 */
									...
									struct rwnx_sta *sta, struct rwnx_txq *txq)
{
	...
	if (!txq->amsdu_len || rwnx_hw->mod_params->amsdu_maxnb < 2 ||
	    ...
	    //printk("no am:%d, %d\n", txq->amsdu_len, rwnx_hw->mod_params->amsdu_maxnb);
		return false;
           }
...
netdev_tx_t rwnx_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
...
#if 0//def CONFIG_RWNX_AMSDUS_TX
    if(eth->h_proto != 0x8e88)
	     if (rwnx_amsdu_add_subframe(rwnx_hw, skb, sta, txq))
		    return NETDEV_TX_OK;
#endif
    ...
    memset(sw_txhdr, 0, sizeof(struct rwnx_sw_txhdr));
	sw_txhdr->txq       = txq;
	sw_txhdr->frame_len = RWNX_TX_DMA_MAP_LEN(skb);
	sw_txhdr->rwnx_sta  = sta;
	sw_txhdr->rwnx_vif  = rwnx_vif;
...
//    eth = (struct ethhdr *)skb->data;
    //skb_put(skb, 4);
    //u32 pattern = 0x123456ac;
    //memcpy((skb->data + skb->len - 4), (u8 *)&pattern, 4);
    if (unlikely(rwnx_prep_dma_tx(rwnx_hw, sw_txhdr, eth + 1)))
        goto free;
...
void rwnx_probersp_work(struct work_struct *work)
{
	...
	if (aicwf_band_steering_block_chk(rwnx_vif, rsp->da)) {
		AICWFDBG(LOGSTEER, "pcie %s, %d, probe_rsp refuse temp  %pM\n", __func__, rwnx_vif->ap.freq, rsp->da);
		goto free_use;
	}
	...
	*buf++ = 0;
	...
	/* Prepare IPC buffer for DMA transfer */
	if (unlikely(rwnx_prep_dma_tx(rwnx_hw, sw_txhdr, buf_rd))) {
#ifdef CONFIG_CACHE_GUARD
		kmem_cache_free(rwnx_hw->sw_txhdr_cache, sw_txhdr);
#else
		kfree(sw_txhdr);
	...
	spin_lock_bh(&rwnx_hw->tx_lock);
	AICWFDBG(LOGSTEER, "pcie p_rsp: %pM", mgmt->da);
...
int rwnx_txdatacfm(void *pthis, void *host_id, u8 free)
{
	...
	if (txq->idx != TXQ_INACTIVE) {
		...
		if (txhdr->hw_hdr.cfm.credits) {
			q->credits += txhdr->hw_hdr.cfm.credits;
			if (txq->credits <= 0)
				rwnx_txq_stop(txq, RWNX_TXQ_STOP_FULL);
