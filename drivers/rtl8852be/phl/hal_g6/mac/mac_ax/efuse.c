			  ...
			  enum mac_ax_efuse_read_cfg cfg, u8 *efuse_map)
{
	...
	ret = efuse_proc_ck(adapter, false);
	if (ret != 0)
		return ret;
			   ...
			   struct mac_ax_pg_efuse_info *info)
{
	...
	ret = efuse_proc_ck(adapter, true);
	if (ret != MACSUCCESS)
		return ret;
		     ...
		     struct mac_ax_pg_efuse_info *info, u8 *phy_map)
{
	...
	if (info->efuse_map_size != adapter->hw_info->log_efuse_size)
		map_size = adapter->efuse_param.limit_efuse_size;
			    ...
			    u8 *efuse_map, bool is_limit)
{
	...
	if (chip_id == MAC_AX_CHIP_ID_8852A) {
		...
		PLTFM_MSG_ALWAYS("efuse log map ver: %X\n", *(efuse_map + EFUSE_MAP_VER_OFFSET_AX));
	...
	if (*bank_efuse_info->log_map_valid == 0) {
		...
		if (*bank_efuse_info->phy_map_valid == 1) {
			...
			PLTFM_MEMCPY(*bank_efuse_info->log_map, map,
				     efuse_size);
			if (parser_cfg == MAC_AX_EFUSE_PARSER_MAP)
...
u32 mac_write_log_efuse(struct mac_ax_adapter *adapter, u32 addr, u8 val)
{
	...
	PLTFM_MSG_ALWAYS("chk log efuse!\n");
			     ...
			     u8 *autoload_status)
{
	...
	val = MAC_REG_R16(R_AX_SYS_WL_EFUSE_CTRL);

	if (val & B_AX_AUTOLOAD_SUS) {
		*autoload_status = AUTOLOAD_SUS;
	} else {
		*autoload_status = AUTOLOAD_FAIL;
		PLTFM_MSG_ERR("[ERR]autoload fail, reg=0x%x!!\n", val);
	}
	return MACSUCCESS;
}
...
MAC_AX_8852BT_SUPPORT)
	...
	/*
	 * The "Efuse Aid" field needs to support existing AX ICs. If the value read from the
	 * efuse is equal to 0xFFFF, Halmac will convert the "adie_id" to a known RL AID.
	 * If the value read from the efuse is not equal to 0xFFFF, Halmac will assign the
	 * "adie_id" as a known RL AID
	 */
	if (efuse_aid != 0xFFFF) {
		switch (efuse_aid) {
		case RL_AID_NULL:
			adapter->hw_info->aid = RL_AID_NULL;
			break;
		case RL_AID_6667:
			adapter->hw_info->aid = RL_AID_6667;
			break;
		case RL_AID_6758:
			adapter->hw_info->aid = RL_AID_6758;
			break;
		case RL_AID_6816:
			adapter->hw_info->aid = RL_AID_6816;
			break;
		case RL_AID_6967:
			adapter->hw_info->aid = RL_AID_6967;
			break;
		default:
			PLTFM_MSG_ERR("adie_id (before transfer) = 0x%x\n", efuse_aid);
			adapter->hw_info->aid = RL_AID_MAX;
			break;
		}
	} else {
...
{
	u32 ret = MACSUCCESS;
	u8 ft_test_pattern0, ft_test_pattern1;
	struct mac_ax_efuse_param *efuse_param = &adapter->efuse_param;
	struct mac_ax_ops *mac_ops = adapter_to_mac_ops(adapter);
	struct mac_ax_hw_info *hw_info = adapter->hw_info;
	u32 ofset0 = hw_info->wl_efuse_size - 2;
	u32 ofset1 = hw_info->wl_efuse_size - 1;

	ret = mac_ops->read_efuse(adapter, ofset0, 1, &ft_test_pattern0,
				  MAC_AX_EFUSE_BANK_WIFI);
	if (ret != MACSUCCESS) {
		PLTFM_MSG_ERR("[ERR] get_hidden_valid read_efuse fail!\n");
		return ret;
	}
	ret = mac_ops->read_efuse(adapter, ofset1, 1, &ft_test_pattern1,
				  MAC_AX_EFUSE_BANK_WIFI);
	if (ret != MACSUCCESS) {
		PLTFM_MSG_ERR("[ERR] get_hidden_valid read_efuse fail!\n");
		return ret;
	...
	if (ft_test_pattern0 == 0x0C && ft_test_pattern1 == 0x00) {
		efuse_param->hidden_valid = 1;
		PLTFM_MSG_ALWAYS("Hidden valid = 1\n");
	} else {
		efuse_param->hidden_valid = 0;
		PLTFM_MSG_WARN("[WARN] Hidden valid = 0!! pattern = %x %x, ofset = %x %x\n",
			       ft_test_pattern0, ft_test_pattern1, ofset0, ofset1);
	}
...
u32 efuse_proc_ck(struct mac_ax_adapter *adapter, bool is_write)
{
	...
	if (adapter->sm.pwr != MAC_AX_PWR_ON) {
		PLTFM_MSG_WARN("[WARN]efuse_proc_ck : sm.pwr=%x, is_write : %d\n",
			       adapter->sm.pwr, is_write);
		if (is_write)
			return MACPWRSTAT;
	}
...
u32 write_hw_efuse(struct mac_ax_adapter *adapter, u32 offset, u8 value)
{
	...
	if (adapter->env_info.env == DUT_ENV_FT2)
		return MACSUCCESS;
			     ...
			     bool is_write)
{
	...
	if (!dav_exist) {
		PLTFM_MSG_WARN("[WARN]No support Adie!");
		return MACSUCCESS;
	} else {
